...
class Vendor(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    business_shortcode = db.Column(db.String(20), unique=True, nullable=False)
...
class ScanLog(db.Model):
    ...
    timestamp = db.Colimn(db.Datetime, default = lambda : datetime.now(timezone.utc))

    qr_id = db.Column(db.Integer, db.ForeignKey("qr_codes.id"), nullable=False)
    user_id = db.Column(db.Integr, db.ForeignKey("users.id"), nullable=False)
